var jogarbot;
class Jardim extends Phaser.Scene {
  constructor() {
    super({key: "Jardim"});
  }
  ...
  create() {
    ...
    //beergame
    beerpapel = this.add.image(center[0]-300,center[1]+370,"beerpapel").setScale(0.3).setInteractive().setRotation(0.2);
    var stylish = { font: "32px Britannic Bold", fill: "#ffffff", align: "center" };
    jogarbot = this.add.text(center[0]+225,center[1]-35,"JOGAR",stylish).setInteractive();
    jogarbot.alpha=0;
    ...
    beerpapel.on("pointerover", function (ev) {
      document.body.style.cursor = 'pointer';
    });
    beerpapel.on("pointerdown", function (ev) {
      if(jogarbot.alpha==0){
        beerpapel.setScale(1.7).setPosition(center[0],center[1]).setRotation(0);
        jogarbot.alpha=1;
      }else{
        beerpapel.setScale(0.3).setPosition(center[0]-300,center[1]+370).setRotation(0.2);
        jogarbot.alpha=0;
      }
    })
    jogarbot.on("pointerover", function (ev) {
      document.body.style.cursor = 'pointer';
    });
    jogarbot.on("pointerdown", function (ev) {
      untz=1;
    })
  ...
  update() {
    ...
    }else if(cursors.right.isDown && x <= 900){
        player.setVelocityX(vel);
    }else if(cursors.right.isDown && x>900){
      torre = locais[2];
      posicao = locais[7];
