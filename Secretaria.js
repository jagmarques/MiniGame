...
class Secretaria extends Phaser.Scene {
  constructor() {
    super({key: "Secretaria"});
    this.cartao = new Image();
    this.aux = 0;
    this.txt = new Text();
  }
  ...
  create() {
    ...
    //Background
    this.add.image(center[0], center[1], "secretariafundo");
    this.txt = this.add.text(center[0]-150,center[1]-300,"JÁ TENS O TEU CARTÃO DA UC\n (verifica na tua mochila)").setTint(0x990011);
    //lixo
    var vazio = this.add.image(center[0]+490,center[1]+220,"lixovazio").setScale(0.3);
    var cheio = this.add.image(center[0]+490,center[1]+220,"lixocheio").setScale(0.3);
    var trash0 = this.add.sprite(center[0]+300,center[1]+420,"trash0").setRotation(55).setInteractive();
    var trash1 = this.add.sprite(center[0]-10,center[1]+400,"trash1").setInteractive().setScale(1.5);
    var trash2 = this.add.sprite(center[0]-300,center[1]+370,"trash2").setInteractive().setScale(0.7);
    ...
    //Player
    if(menuNumber===5){
      player = this.physics.add.image(center[0]-500,center[1]+200, 'personagem1');
      if(semestre == 1){
        papelcartao = this.add.image(center[0]-150,center[1]-150,"papelcartao").setScale(0.4).setInteractive();
      }
      this.cartao = this.add.image(center[0], center[1], "cartaoUC1").setScale(0.5);
      this.cartao.alpha=0;
    }
    if(menuNumber===6){
      player = this.physics.add.image(center[0]-500,center[1]+200, 'personagem2');
      if(semestre == 1){
        papelcartao = this.add.image(center[0]-150,center[1]-150,"papelcartao").setScale(0.4).setInteractive();
      }
      this.cartao = this.add.image(center[0], center[1], "cartaoUC2").setScale(0.5);
      this.cartao.alpha=0;
    }
    ...
    if(trash_obj>0){
      if(semestre == 1 && secretariaLimpo==0){
    ...
    //cartao
    papelcartao.on("pointerover", function (ev) {
      document.body.style.cursor = 'pointer';
    })
    var acum=0;
    papelcartao.on("pointerdown", function (ev) {
      if(acum==0){
      papelcartao.setScale(1.2).setPosition(center[0],center[1]);
      acum=1;
    }else{
      papelcartao.setScale(0.3).setPosition(center[0]-150,center[1]-150);
      acum=0;
    }})
